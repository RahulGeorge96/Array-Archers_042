...
export const LoginPage = ()=>{
  ...
  function handeSubmit (e){
    ...
    axios.get("https://bike-enthusiast-default-rtdb.asia-southeast1.firebasedatabase.app/user.json")
      .then((res)=>{
        let filterdata = Object.entries(res.data).filter(([key, e])=>{
          return emailref.current.value == e.emailID && passwordref.current.value == e.password
        })
        if(filterdata.length>0){
          localStorage.setItem("currloginuser", JSON.stringify(filterdata))
          alert("Login Sucess");
    ...
    <div className="loginpagecom">
        ...
        <form id="form" onSubmit={handeSubmit}>
          ...
          <div className="social">
            <div onClick={handleClickGoogle} style={{cursor:"pointer"}} className="go"><i className="fab fa-google" onClick={handleClickGoogle} />  Google</div>
            <div onClick={handleClickGoogle} style={{cursor:"pointer"}} className="fb"><i onClick={handleClickGoogle} className="fab fa-facebook" />  Facebook</div>
          </div>
        </form>
      </div>
  )
